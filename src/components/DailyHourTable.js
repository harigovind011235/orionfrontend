...
function DailyHourTable() {
  ...
  const employeeDailyHour =
    employeedailyhours && employeedailyhours["results"]
      ? employeedailyhours["results"]
      : null;
  const totalDailyHourCount =
    employeedailyhours && employeedailyhours["count"]
      ? employeedailyhours["count"]
      : null;
    ...
    <Container className="p-2 mt-4">
      <Row md={6} lg={6} className="mt-4">
        <Col lg={12} md={12}>
          {/* <InfiniteScroll
            dataLength={
              dailyhoursarray && dailyhoursarray.length > totalDailyHourCount
                ? dailyhoursarray.length
                : null
            }
            next={fetchData}
            hasMore={hasMore}
                ...
                dailyhoursarray.map((each) => (
                  <tr key={each.id}>
                    ...
                    <td
                      className={
                        each.hours_perday
                          ? checkWorkHour(each.hours_perday)
                          : ""
                      }
                    >
                      {each.hours_perday}
