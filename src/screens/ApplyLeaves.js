...
function ApplyLeaves() {
  ...
  const navigate = useNavigate();
    ...
    const submitHandler = (event) => {
      ...
      dispatch(EmployeeLeaveApply(leaveDate,leaveType,leaveNotes,noofleaves))
  ...
  return (
    <Container>
        ...
        <Row md={12} lg={12} className="justify-content-lg-center justify-content-md-center">
        <Col lg="auto" md="auto">
        {leaveapplied && leaveapplied === true ? <Alert variant="success">Leave Applied Successfully</Alert> : null}
          {loading && <Loader/>}
          {error && <Message variant="danger">Leave Submission Failed</Message>}
        <Form className='mt-4 p-4' onSubmit={submitHandler}>
      <Form.Group className="mb-3">
        <Form.Label>Date</Form.Label>
        <Form.Control type="date" placeholder="Enter the date" value={leaveDate} onChange={(e) => setLeaveDate(e.target.value)} />
      </Form.Group>

      <Form.Label>Type Of Leave</Form.Label>
      <Form.Select aria-label="Default select example" value={leaveType} onChange={(e) => setLeaveType(e.target.value)}>
    ...
    <Form.Control type="number" min={1} value={noofleaves} onChange={(e) => setNoOfLeave(e.target.value)}/>
  </Form.Group>
    <label htmlFor="specialrequesttextarea" className='mt-4'>Any Special Request</label>
      <textarea
      ...
      />
      <Button variant="primary" type="submit" className='mt-4 offset-lg-3'>
        Submit
      </Button>
    </Form>
        </Col>
